import React, { useState, useEffect } from "react";
import axios from "axios";
import {
  AreaChart, Area, LabelList, XAxis, YAxis, Tooltip, ResponsiveContainer,
} from 'recharts';
import RemoveCircleTwoToneIcon from '@material-ui/icons/RemoveCircleTwoTone';
import AddCircleTwoToneIcon from '@material-ui/icons/AddCircleTwoTone';

const Home = props => {
  const [bitcoin, setBitcoin] = useState([]);
  const [bitconnect, setBitconnect] = useState([]);

  useEffect(() => {
    axios({
      method: "get",
      url: `http://localhost:1337/crypto/`
    })
      .then(response => {
        setBitcoin(response.data.response[0]);
        setBitconnect(response.data.response[1]);
      })
      .catch(error => {
        console.log(error);
      });
  }, [])

  const data = [
    {
      name: 'Page A',
      uv: 4000,
      pv: 2400,
      amt: 2400,
    },
    {
      name: 'Page B',
      uv: 3000,
      pv: 1398,
      amt: 2210,
    },
    {
      name: 'Page C',
      uv: 2000,
      pv: 9800,
      amt: 2290,
    },
    {
      name: 'Page D',
      uv: 2780,
      pv: 3908,
      amt: 2000,
    },
    {
      name: 'Page E',
      uv: 1890,
      pv: 4800,
      amt: 2181,
    },
    {
      name: 'Page F',
      uv: 2390,
      pv: 3800,
      amt: 2500,
    },
    {
      name: 'Page G',
      uv: 3490,
      pv: 4300,
      amt: 2100,
    },
  ];

  return (
    <>
      <div className="main_main">
        <div className="chartcards_wrapper">

          <div className="chartcard_first">
            <div className="chartcard_wrapper">
              <div className="chartcard_top">
                <span className="chartcard_title">{bitcoin.currency}</span>
                <span className="chartcard_add">
                  <AddCircleTwoToneIcon fontSize="large" />
                </span>
              </div>
              <div className="chartcard_bottom">
                <span className="chartcard_value">{bitcoin.value} Kr</span>
                <span className="chartcard_remove">
                  <RemoveCircleTwoToneIcon fontSize="large" />
                </span>
              </div>
            </div>
          </div>


          <div className="chartcard_second">
            <div className="chartcard_wrapper">
              <div className="chartcard_top">
                <span className="chartcard_title">{bitconnect.currency}</span>
                <span className="chartcard_add">
                  <AddCircleTwoToneIcon fontSize="large" />
                </span>
              </div>
              <div className="chartcard_bottom">
                <span className="chartcard_value">{bitconnect.value} Kr</span>
                <span className="chartcard_remove">
                  <RemoveCircleTwoToneIcon fontSize="large" />
                </span>
              </div>
            </div>
          </div>

        </div>
        <ResponsiveContainer>
          <AreaChart
            data={data}
            margin={{
              top: 10, right: 30, left: 0, bottom: 0,
            }}
          >
            <defs>
              <linearGradient id="colorUv" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#8884d8" stopOpacity={0.8} />
                <stop offset="95%" stopColor="#8884d8" stopOpacity={0} />
              </linearGradient>
              <linearGradient id="colorPv" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#82ca9d" stopOpacity={0.8} />
                <stop offset="95%" stopColor="#82ca9d" stopOpacity={0} />
              </linearGradient>
            </defs>
            {/*}
            <Area type="monotone" dataKey="uv" stroke="white" fillOpacity={1} fill="url(#colorUv)" />
               
          {*/}
            <YAxis stroke="white" />
            <XAxis dataKey="name" stroke="white" />
            <Tooltip />
            <LabelList
              className="label"
              dataKey="name"
              position="insideTop"
              angle="45" />
            <Area
              type="monotone"
              dataKey="pv"
              stroke="#fff"
              fillOpacity={1}
              fill="url(#colorPv)" />
            <Area
              type="monotone"
              dataKey="uv"
              stroke="#fff"
              fillOpacity={1}
              fill="url(#colorUv)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </>
  );
};

export default Home;
